# backend/routes/ses.py
# View Form & API for engine.tsf_ses_m_a0
...
VIEW_NAME = "engine.tsf_ses_m_a0"

router = APIRouter(prefix="/ses", tags=["ses-view"])
  ...
  <body>
    ...
    <script>
      ...
      function updateExportHref(){{
        ...
        el('csv').dataset.href = '/ses/export?' + q.toString();
      ...
      async function doLoad(){{
        try{{
          ...
          const r = await fetch('/ses/query', {{ method:'POST', headers:{{'content-type':'application/json'}}, body: JSON.stringify(payload) }});
...
def export_csv(forecast_id: str = FQuery(...), date_from: Optional[str] = None, date_to: Optional[str] = None):
    ...
    filename = f"tsf_export_ses_{forecast_id}.csv"
